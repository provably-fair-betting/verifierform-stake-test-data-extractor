...
export function parseCssColor(value: unknown): RgbTuple | null {
  ...
  return rgbMatch.slice(1).map((component) => Number.parseInt(component, 10)) as RgbTuple;
}

export function parseHexColor(hexColor: string): RgbTuple {
  ...
  const [red, green, blue] = hexMatch[1]
    .match(/../gu)!
    .map((component) => Number.parseInt(component, 16)) as [number, number, number];
