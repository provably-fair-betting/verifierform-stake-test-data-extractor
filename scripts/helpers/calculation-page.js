...
async function waitForCalculationForm(
  ...
  initialPage,
  logger,
  { waitTimeoutMs, waitIntervalMs, stakeLoaderSelector },
) {
  const startedAt = Date.now();
  let didLogBlockingPage = false;
  let didLogStakeLoader = false;
  let lastInitialPageState = null;
...
async function readCalculationPageState(page, stakeLoaderSelector) {
  try {
    return await page.evaluate((loaderSelector) => {
      ...
      function hasVisibleLoader(selector) {
...
function isCalculationFormReady(pageState) {
  return (
...
function isCalculationPage(pageState) {
  return (
...
function looksLikeChallengePage(pageState) {
