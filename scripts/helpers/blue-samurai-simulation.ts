...
import type { BlueSamuraiRound, BlueSamuraiSymbol } from "../types.js";
...
export const BLUE_SAMURAI_CATEGORY_ORDER: BlueSamuraiCategory[] = [
  "ordinary",
  "bonus",
  "special-rounds",
  "bonus-with-retrigger",
...
const BLUE_SAMURAI_REGULAR_COLUMN_SIZES = [3, 4, 4, 4, 3] as const;
const BLUE_SAMURAI_SPECIAL_COLUMN_SIZES = [4, 4, 4] as const;
...
function buildProbabilityTable(columnName: "outer" | "inner"): ProbabilityEntry[] {
  let running = 0;

  type ProbabilityRow = { symbol: string; outer: number; inner: number };

  return (probabilities as ProbabilityRow[])
...
export function simulateBlueSamuraiRounds({ clientSeed, serverSeed, nonce }: SeedContext): BlueSamuraiRound[] {
  const floatGenerator = createFloatGenerator({
    clientSeed,
    serverSeed,
    nonce,
  });
  const rounds: BlueSamuraiRound[] = [];
  let bonusSpin = 0;
  let remainingRounds = 1;
  let remainingSpecialRounds = 0;
  let roundIndex = 0;
  let stuckSamuraiIndices = new Set<number>();
  ...
  while (remainingRounds > 0) {
    ...
    rounds.push({
      ...
      retriggerType: retriggerType as "bonus" | "special" | null,
      specialRound,
...
function buildRoundSymbols({
  ...
  floatGenerator: Generator<number>;
  specialRound: boolean;
  stuckSamuraiIndices: Set<number>;
}): BlueSamuraiSymbol[] {
...
function resolveBlueSamuraiSymbol(float: number, reelType: "outer" | "inner"): string {
...
function isSpecialTriggerRound(symbols: BlueSamuraiSymbol[]): boolean {
...
function updateStuckSamuraiIndices(
...
function isOuterPosition(index: number): boolean {
  return index < 3 || index > 14;
}

function differenceOfSets(left: Set<number>, right: Set<number>): Set<number> {
  return new Set([...left].filter((value) => !right.has(value)));
}

function buildFloatColumns(
  symbols: BlueSamuraiSymbol[],
  specialRound: boolean,
): (number | null)[][] {
...
export function classifyBlueSamuraiRounds(rounds: BlueSamuraiRound[]): BlueSamuraiCategory | null {
