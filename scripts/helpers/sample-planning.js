export const resolveConfigValue = async (value, context) =>
 ...
 */
export const buildSelectValueCombinations = (selects = []) => {
  if (selects.length === 0) {
    return [{}];
  }
...
const expandCombinationsForSelect = (combinations, select) => {
...
const assertSupportedSelectConfig = (select) => {
...
export const getEffectiveSampleCount = (
  ...
  defaultNonceCount,
  selectCombinations,
) => Math.max(game.nonceCount ?? defaultNonceCount, selectCombinations.length);
...
export const resolveSlotCategoryCounts = (
  ...
  const retriggerCount =
    slotSampleCountOverrides?.retriggerCount ??
    game.sampleCategoryDefaults.retrigger;
