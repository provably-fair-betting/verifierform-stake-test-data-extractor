...
function resolveBlueSamuraiCategoryCounts(game, sampleCount) {
...
function assertSupportedBlueSamuraiCategoryDefaults(game, configuredCounts) {
...
function resolveBlueSamuraiCategoryCount(game, key, value) {
  if (value === undefined) {
    return 0;
  }
...
function hasCollectedAllBlueSamuraiTargets(buckets, targets) {
...
export async function readVerifiedBlueSamuraiResult(
  ...
  for (const expectedRound of context.blueSamuraiRounds ?? []) {
    ...
    const verifiedRound = await page.evaluate(
      async ({ headingText, lookupChecks }) => {
...
async function readBlueSamuraiCalculatorState(page) {
...
function buildLookupChecks(round) {
...
function buildExpectedLookupSymbols(round) {
  if (round.specialRound) {
    return [];
  }
...
export function parseBlueSamuraiResult(rawResult, context) {
...
function sameNumberMatrix(left, right) {
  return (
    Array.isArray(left) &&
    Array.isArray(right) &&
