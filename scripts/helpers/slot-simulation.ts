...
import type { SlotRound, VisibleReelSymbols } from "../types.js";
...
type SlotCategory = "simple" | "bonus" | "retrigger";

type SlotBuckets = Record<SlotCategory, SlotSampleResult[]>;
type SlotTargets = Record<SlotCategory, number>;

type FindSlotSampleResultsOptions = {
  clientSeed: string;
  serverSeed: string;
  sampleCount: number;
  bonusCount: number;
...
type ClassifySlotSpinResult = {
  rounds: SlotRound[];
  type: SlotCategory;
};

const SLOT_REEL_SIZES = [30, 30, 30, 30, 41] as const;
...
}: FindSlotSampleResultsOptions): SlotSampleResult[] {
...
function assertSlotSampleTargets({
  sampleCount,
  bonusCount,
  retriggerCount,
}: {
...
export function classifySlotSpin(seed: SeedContext): ClassifySlotSpinResult {
...
export function simulateSlotRounds({ clientSeed, serverSeed, nonce }: SeedContext): SlotRound[] {
...
export function buildVisibleReelSymbols(
  reelIndex: number,
  centerIndex: number,
): VisibleReelSymbols {
  const reel = (slotReels as string[][])[reelIndex];
...
function countScatterSymbols(visibleSymbolsByReel: VisibleReelSymbols[]): number {
...
function isScatter(symbol: string): boolean {
  return symbol === SCATTER_SYMBOL;
}

function hasCollectedAllTargets(buckets: SlotBuckets, targets: SlotTargets): boolean {
  return Object.entries(targets).every(
    ([key, targetCount]) => buckets[key as SlotCategory].length >= targetCount,
  );
}
